...
# ---------- 配置 ----------
DATA_PATH = "../data/processed/exp_b_9class/train.csv"
FIG_DIR = "../data/figures/"
STOPWORDS_PATH = "../data/processed/exp_b_9class/stopwords.txt"
...
plt.title(f'商品类别分布（共{df["cat"].nunique()}个类别）', fontsize=13)
...
print("图3：类别×标签热力图...")
plt.figure(figsize=(9, 6))
heatmap_data = pd.crosstab(df['cat'], df['label'], normalize='index') * 100
...
print("图6：按类别分的评论长度...")
plt.figure(figsize=(9, 6))
cat_length = df.groupby('cat')['review_length'].mean().sort_values()
