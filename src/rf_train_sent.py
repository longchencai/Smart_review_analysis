...
x_train_text, x_test_text, y_train, y_test = train_test_split(
    df['words'], df['label'], test_size=0.2, random_state=22, stratify=df['label']
)
...
# 第六步：保存模型和向量化器
print("保存模型和向量化器...")
with open(f"{SAVE_DIR}/rf_sent_model.pkl", 'wb') as f:
    pickle.dump(model, f)
with open(f"{SAVE_DIR}/tfidf_sent_vectorizer.pkl", 'wb') as f:
    pickle.dump(tfidf, f)
