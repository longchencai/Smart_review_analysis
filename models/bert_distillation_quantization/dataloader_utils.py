...
def load_data_list(csv_path):
    ...
    data_list = []
    # 读取 csv
    # 显式用 utf-8-sig：数据 CSV 由清洗脚本以 utf-8-sig 落盘（带 BOM），
    # 不指定会让首列名变成 '\ufeffreview'（部分 pandas 版本会自动跳过 BOM，但不要依赖它）
...
def build_all_dataloader(batch_size=None):
    # batch_size 传 None 时沿用 config.batch_size（保持对原有脚本的行为不变）
    # 评估时传 config.eval_batch_size 可以显著加快速度；推理批次大小不影响指标
    bs = batch_size or config.batch_size
    # 1.构建 train_dataloader
    data_list = load_data_list(config.train_path)
    data_set = MyDataSet(data_list)
    train_dataloader = torch.utils.data.DataLoader(data_set, bs, shuffle=True, collate_fn=my_collate_fn)
    # 2.构建 val_dataloader
    data_list = load_data_list(config.val_path)
    data_set = MyDataSet(data_list)
    val_dataloader = torch.utils.data.DataLoader(data_set, bs, shuffle=False, collate_fn=my_collate_fn)
    # 3.构建 test_dataloader
    data_list = load_data_list(config.test_path)
    data_set = MyDataSet(data_list)
    test_dataloader = torch.utils.data.DataLoader(data_set, bs, shuffle=False, collate_fn=my_collate_fn)
    # 4.返回
